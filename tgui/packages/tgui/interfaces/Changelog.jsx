...
export class Changelog extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: 'Загружаем данные списка изменений...',
      selectedDate: '',
      selectedIndex: 0,
    };
    this.dateChoices = [];
  ...
  getData = (date, attemptNumber = 1) => {
    ...
    if (attemptNumber > maxAttempts) {
      return this.setData(`Не удалось загрузить данные после ${maxAttempts} попыток.`);
    }
    ...
    fetch(resolveAsset(`${date}.yml`)).then(async (changelogData) => {
      ...
      if (errorRegex.test(result)) {
        ...
        self.setData(`Загружаем данные списка изменений${'.'.repeat(attemptNumber + 3)}`);
        setTimeout(() => {
        ...
        <Stack.Item>
          <Button
            ...
            onClick={() => {
              ...
              this.setData('Загружаем данные списка изменений...');
