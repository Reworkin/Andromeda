/// Синхронизируйте это с JOBS_RU из tgui\packages\tgui\components\ru_jobs.tsx
...
// Используйте его ТОЛЬКО для отображения!! Там где указывается должность, заменить на "job_title_ru()", чтоб указывалось на Русском.
