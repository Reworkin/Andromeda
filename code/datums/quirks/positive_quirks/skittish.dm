/datum/quirk/skittish
	name = "Skittish"
	desc = "(Пугливый) - Вас легко испугать, и вы часто прячетесь. Вбегите в закрытый шкафчик, чтобы запрыгнуть в него, если у вас есть доступ к нему. Вы можете ходить, чтобы избежать этого."
	icon = FA_ICON_TRASH
	value = 8
	mob_trait = TRAIT_SKITTISH
	medical_record_text = "Пациент демонстрирует крайне сильное стремление избежать опасности и рассказывает как прятался в контейнерах от страха."
	mail_goodies = list(/obj/structure/closet/cardboard)
