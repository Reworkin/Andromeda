/datum/quirk/erratic
	name = "Erratic"
	desc = "(Перепады настроения) - Ваше настроение колеблется как маятник, заставляя вашу личность меняться по прихоти время от времени."
	icon = FA_ICON_MASKS_THEATER
	value = -3
	gain_text = span_danger("Вы чувствуете себя неустойчиво.") // повторите это?
	lose_text = span_notice("Вы чувствуете себя более стабильно.")
	...
	/// Время отката между случайными сменами личности
...
/datum/quirk/erratic/proc/randomize_personalities()
	...
	if(random_index % 2 == 0)
		random_index = 0
		replace_personalities(base_personalities)
		to_chat(quirk_holder, span_notice("Вы чувствуете... нормально."))
		announce_personality_change()
		return
	...
	to_chat(quirk_holder, span_notice("Вы чувствуете... иначе."))
	announce_personality_change()
