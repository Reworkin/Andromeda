...
/datum/quirk/item_quirk/food_allergic
	name = "Food Allergy"
	desc = "(Аллергия) - С самого детства у вас была аллергия на некоторые продукты."
	icon = FA_ICON_SHRIMP
	value = -2
...
/datum/quirk/item_quirk/food_allergic/add_unique(client/client_source)
	...
	give_item_to_holder(dogtag, list(LOCATION_BACKPACK, LOCATION_HANDS), flavour_text = "Держите его поблизости находясь в столовой.", notify_player = TRUE)
