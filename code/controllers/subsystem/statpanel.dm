...
/datum/controller/subsystem/statpanels/fire(resumed = FALSE)
	if (!resumed)
		...
		global_data += list(
			"ID раунда: [GLOB.round_id ? GLOB.round_id : "Не установлено"]",
			"Время с начала раунда: [ROUND_TIME()]",
			"Время на станции: [station_time_timestamp()]",
			"Серверное время: [time2text(world.timeofday, "YYYY-MM-DD hh:mm:ss", world.timezone)]",
			"Дилатация времени: [round(SStime_track.time_dilation_current,1)]% В среднем:([round(SStime_track.time_dilation_avg_fast,1)]%, [round(SStime_track.time_dilation_avg,1)]%, [round(SStime_track.time_dilation_avg_slow,1)]%)",
			"Игроков онлайн: [LAZYLEN(GLOB.clients)]",
		)
		...
		if(SSticker.reboot_timer)
			...
			if(reboot_time)
				global_data += "Перезапуск раунда через: [DisplayTimeText(reboot_time, 1)]"
